...
class Database:
    ...
    async def create_user(self, telegram_id: int, username, full_name, referrer_id=None):
        ...
        async with self.pool.acquire() as conn:
            ...
            if referrer_id and referrer_id != telegram_id:
                try:
                    ...
                    await conn.execute(
                        "UPDATE users SET balance = balance + 100 WHERE telegram_id = $1",
                        referrer_id)
    ...
    async def update_user_status(self, telegram_id: int, status: str, months: int = None):
        async with self.pool.acquire() as conn:
            if status == "premium_forever":
                await conn.execute(
                    "UPDATE users SET status='premium_forever', trial_until=NULL, "
                    "premium_until=NULL WHERE telegram_id=$1", telegram_id)
                ...
                await conn.execute(
                    "UPDATE users SET status='premium', premium_until=$1, "
                    "trial_until=NULL WHERE telegram_id=$2",
                    premium_until, telegram_id)
            else:
                await conn.execute(
                    "UPDATE users SET status=$1, trial_until=NULL, "
                    "premium_until=NULL WHERE telegram_id=$2", status, telegram_id)
    ...
    async def get_today_orders_count(self, telegram_id: int) -> int:
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "SELECT COUNT(*) FROM orders WHERE user_id=$1 AND DATE(created_at)=CURRENT_DATE",
                telegram_id)

    async def create_order(self, telegram_id: int, category: str, description: str, contacts: str):
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    async def create_promocode(self, code: str, action: str, value: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO promocodes (code, action, value) VALUES ($1, $2, $3)",
                code, action, value)

    async def use_promocode(self, code: str, user_id: int) -> Optional[Dict]:
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM promocodes WHERE code=$1 AND used_by IS NULL", code)
            if not row:
                return None
            await conn.execute(
                "UPDATE promocodes SET used_by=$1 WHERE code=$2", user_id, code)
            return dict(row)
    ...
    async def create_ad(self, user_id: int, ad_text: str, media_type: str,
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                "button_text, button_url) VALUES ($1, $2, $3, $4, $5, $6)",
    ...
    async def mark_last_ad_paid(self, user_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                "WHERE id = (SELECT id FROM ads WHERE user_id=$1 AND status='pending' "
                "ORDER BY id DESC LIMIT 1)", user_id)

    async def create_parsed_order(self, source_chat, source_chat_title, author_id,
                                   author_username, author_name, message_id,
                                   message_text, message_link, category):
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                "INSERT INTO parsed_orders (source_chat, source_chat_title, author_id, "
                "author_username, author_name, message_id, message_text, message_link, category) "
