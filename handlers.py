...
from config import (ADMIN_IDS, PRICES, ORDER_CATEGORIES, CATEGORIES,
                    SUPPORT_URL, AD_PRICE_RUB, TON_ADDRESS)
from database import Database
from keyboards import (main_keyboard, admin_keyboard, categories_choice_keyboard,
                       ad_creation_keyboard, premium_keyboard, categories_keyboard,
                       cancel_keyboard)
...
async def ad_process_button(message: types.Message, state: FSMContext):
    try:
        ...
        if not button_text:
            raise ValueError
        await state.update_data(ad_button={"text": button_text, "url": button_url})
        await message.answer(f"✅ Кнопка: {button_text}",
...
async def ad_publish(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if not data.get('ad_text') and not data.get('ad_media'):
    ...
    with Database() as db:
        db.create_ad(
            ...
            data.get('ad_media', {}).get('file_id', ''),
            data.get('ad_button', {}).get('text', ''),
            data.get('ad_button', {}).get('url', ''),
        )

    text = (
        f"🚀 Разместить рекламу\n\n"
        f"Сумма: {AD_PRICE_RUB} ₽ ({total_ton} TON)\n\n"
        f"💳 Отправьте {total_ton} TON на адрес:\n"
        f"{TON_ADDRESS}\n\n"
        f"📝 В комментарии укажите:\n{comment}\n\n"
        f"⚠️ Без комментария платеж не засчитается!\n\n"
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Я оплатил",
                              callback_data=f"check_ad_{user_id}")],
        [InlineKeyboardButton(text="🏠 Главное меню", callback_data="menu_home")],
    ])
    await safe_edit(callback, text, kb)
    await callback.answer()
...
async def check_ad_payment(callback: types.CallbackQuery):
    ...
    if callback.from_user.id != user_id:
        await callback.answer("❌ Не ваш платеж!", show_alert=True)
        return
    await callback.answer("🔄 Проверяем платеж...")

    expected_ton = rub_to_ton(AD_PRICE_RUB)
    expected_comment = f"ad_{user_id}"
    found = await find_ton_payment(expected_comment, expected_ton)
...
async def admin_grant_start(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    await safe_edit(callback, "👑 Введите ID или @username пользователя:",
                    cancel_keyboard())
...
async def admin_grant_user(message: types.Message, state: FSMContext):
    ...
    try:
        await bot.send_message(user['telegram_id'],
                               "👑 Вам выдан ПРЕМИУМ НАВСЕГДА администратором!")
    except Exception:
        pass
    await state.clear()
...
async def admin_revoke_start(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    await safe_edit(callback, "👑 Введите ID или @username для снятия премиума:",
                    cancel_keyboard())
...
async def admin_promo_start(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        ...
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 Премиум 30 дней",
                              callback_data="promoact_premium_30")],
        [InlineKeyboardButton(text="💰 Баланс 100₽",
                              callback_data="promoact_balance_100")],
        [InlineKeyboardButton(text="💰 Баланс 500₽",
                              callback_data="promoact_balance_500")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="menu_admin")],
    ])
    await safe_edit(callback, "🎫 Выберите действие промокода:", kb)
...
async def admin_broadcast(message: types.Message, state: FSMContext):
    ...
    for u in users:
        try:
            ...
            continue
    await message.answer(f"✅ Рассылка завершена: {sent} получателей",
                         reply_markup=admin_keyboard())